...
namespace GS.Service.Customer
{
    public class UserService : BaseService
    {
        ...
        /// <returns></returns>
        public ServiceResult UpdateArea(string userId, AreaBo area)
        {
            var user = Db.Query<User>().FirstOrDefault(a => a.UserId == userId);
            if (user == null)
            {
            ...
            Db.Update<User>(u => u.UserId == userId,
                u => new User
                {
        ...
        /// <param name="expression"></param>
        /// <returns></returns>
        public PageList<SearchUserBo> GetUserPageList(int pageIndex, int pageSize, Expression<Func<User, bool>> expression)
        {
            var list = Db.Query<User>().OrderByDesc(u => u.CreateDatetime).PageList(expression, pageIndex, pageSize).MapPageListTo(a => a.MapTo<SearchUserBo>());

            foreach (var item in list.Items)
            {
                item.StatusName = item.Status == (int)PersonEnum.EUserStatus.正常
                    ? PersonEnum.EUserStatus.正常.GetDisplayName()
                    : PersonEnum.EUserStatus.停用.GetDisplayName();
            }
            return list;
        }
        ...
        /// <returns></returns>
        public ServiceResult UpdateStatus(string userId, int status)
        {
            var user = Db.Query<User>().FirstOrDefault(a => a.UserId == userId);
            if (user == null)
            {
            ...
            Db.Update<User>(u => u.UserId == userId, u => new User { Status = status });
            return ServiceResult.IsSuccess("修改成功");
        }
        ...
        /// </summary>
        /// <param name="userId"></param>
        /// <param name="userLable"></param>
        /// <returns></returns>
        public ServiceResult UserLable(string userId, string userLable)
        {
            var user = Db.Query<User>().FirstOrDefault(a => a.UserId == userId);
            if (user == null)
            {
            ...
            Db.Update<User>(u => u.UserId == userId, u => new User { Userlable = userLable });
            return ServiceResult.IsSuccess("修改成功");
        }
        ...
        /// <returns></returns>
        public ServiceResult ResetPassword(string userId, string password)
        {
            var user = Db.Query<User>().FirstOrDefault(a => a.UserId == userId);
            if (user == null)
            {
            ...
            var pwd = EncodePassword(userId, password);
            Db.Update<User>(u => u.UserId == userId, u => new User { Password = pwd });
            return ServiceResult.IsSuccess("重置密码成功");
        }
        ...
        /// <returns></returns>
        public ServiceResult BusinessManager(string userId, string businessManager)
        {
            var user = Db.Query<User>().FirstOrDefault(a => a.UserId == userId);
            if (user == null)
            {
            ...
            Db.Update<User>(u => u.UserId == userId, u => new User { BusinessManager = businessManager });
            return ServiceResult.IsSuccess("修改成功");
        }
        ...
        /// </summary>
        /// <param name="userId"></param>
        /// <param name="memberGradesId"></param>
        /// <param name="memberGradesName"></param>
        /// <returns></returns>
        public ServiceResult VipLv(string userId, string memberGradesId,string memberGradesName)
        {
            var user = Db.Query<User>().FirstOrDefault(a => a.UserId == userId);
            if (user == null)
            {
            ...
            Db.Update<User>(u => u.UserId == userId, u => new User { MemberGradesId = memberGradesId,VipLv = memberGradesName });
            return ServiceResult.IsSuccess("修改成功");
        }
    }
}
